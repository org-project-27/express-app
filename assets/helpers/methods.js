...
export const $sendResponse = {
    success: (via, statusCode = statusCodes.OK, message = apiMessageKeys.DONE, others = {}) => {
        const tId = setTimeout(() => {
            via.status(statusCode).send({ success: true, message, ...others });
            clearTimeout(tId);
        }, responseTimeout);
    },
    failed: (via, statusCode = statusCodes.BAD_REQUEST, message = apiMessageKeys.SOMETHING_WENT_WRONG, others = {}) => {
        const tId = setTimeout(() => {
